...
if not df_current.empty:
    ...
    jq_df = df_current.groupby("厂家").agg(
        ...
        最长逾期天数=("预计-实际交期的差值", lambda x: -x.min() if x.min() < 0 else 0),
    ).round(2).reset_index()
...
if not df_current.empty:
    ...
    fig.add_trace(go.Scatter(
        ...
        text=factory_chart["准时率(%)"].astype(str)+"%",
...
factory_analysis = df_current.groupby("厂家").agg(
    ...
    # ✅ 修复1：这里改成交期差值（天数）
    平均实际交期=("预计-实际交期的差值", "mean"),
    最长实际交期=("预计-实际交期的差值", lambda x: -x.min() if x.min() < 0 else 0),
...
if df_category.empty:
    ...
    category_stats = df_category.groupby(["厂家", "厂家类目明细"]).agg(
        ...
        # ✅ 修复2：这里也改成交期差值
        平均交期=("预计-实际交期的差值", "mean"),
        最长交期=("预计-实际交期的差值", lambda x: -x.min() if x.min() < 0 else 0)
...
# ✅【唯一修复点】全部改成 预计-实际交期的差值
actual_stats = df_actual.groupby(["厂家", "厂家类目明细"]).agg(
    实际交期80分位=("预计-实际交期的差值", lambda x: biz_quantile(x, 0.8)),
    实际交期85分位=("预计-实际交期的差值", lambda x: biz_quantile(x, 0.85)),
    实际交期90分位=("预计-实际交期的差值", lambda x: biz_quantile(x, 0.9)),
    实际交期95分位=("预计-实际交期的差值", lambda x: biz_quantile(x, 0.95)),
    实际交期100分位=("预计-实际交期的差值", lambda x: biz_quantile(x, 1.0)),
    样本订单数=("采购单号", "count"),
...
df_final["到货年月"] = pd.to_datetime(df_final["到货年月"].astype(str)).dt.to_period("M")
